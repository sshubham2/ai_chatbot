...
class ChatAppCE:
    ...
    def initialize_session_state(self):
        ...
        if "ce_store" not in st.session_state:
            st.session_state.ce_store = {}
        if "ce_messages" not in st.session_state:
            st.session_state.ce_messages = []
        if "ce_config" not in st.session_state:
            st.session_state.ce_config = {"configurable": {"session_id": "abc123"}}
    ...
    def get_session_history(session_id: str) -> BaseChatMessageHistory:
        if session_id not in st.session_state.ce_store:
            st.session_state.ce_store[session_id] = ChatMessageHistory()
        return st.session_state.ce_store[session_id]
    ...
    def display_chat_messages(self):
        for message in st.session_state.ce_messages:
    ...
    def handle_user_input(self, conversational_rag_chain):
        ...
        if prompt:
            st.session_state.ce_messages.append({"role": "user", "content": prompt})
            with st.chat_message("user"):
                st.markdown(prompt)

            with st.chat_message("assistant"):
                try:
                    stream = conversational_rag_chain.stream({"messages": prompt}, config=st.session_state.ce_config)
                    response = st.write_stream(stream)
                except Exception as e:
                    st.info(f"An error occurred: {str(e)}. Please check your API key or try again later.")
                    st.stop()

            st.session_state.ce_messages.append({"role": "assistant", "content": response})
